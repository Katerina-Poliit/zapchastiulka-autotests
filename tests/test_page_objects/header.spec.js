...
test.describe('header.spec', () => {
	...
	CATALOG_MENU_FILTERS_CATEGORIES.forEach((namePage, indx) => {
		test(`TC 01.01.19.2 Verify that the user can navigate to ${namePage} page by clicking on the appropriate buttons in the "Фільтри" menu`, async ({ page }) => {
			const homePage = new HomePage(page);
			await homePage.clickCatalogbutton();
			await homePage.clickFiltersCategoryHover();
			await homePage.clickCatalogMenuButton(namePage);

			await expect(page).toHaveURL(BASE_URL + CATALOG_MENU_FILTERS_URLs_END_POINTS[indx]);
			await expect(page.getByRole('link', { name: namePage, exact: true }).first()).toHaveText(namePage);
			await expect(page.locator('p.inline-block')).toHaveText(namePage);
		})
	...
	CATALOG_MENU_SPARE_PARTS_FOR_AGRICULTURAL_MACHINERY_CATEGORIES.forEach((namePage, indx) => {
		test(`TC 01.01.19.3 Verify that the user can navigate to ${namePage} page by clicking on the appropriate buttons in the "Запчастини до сільгосптехніки" menu`, async ({ page }) => {
			const homePage = new HomePage(page);
			await homePage.clickCatalogbutton();
			await homePage.sparePartsForAgriculturalMachineryHover();
			await homePage.clickCatalogMenuButton2(namePage);

			await expect(page).toHaveURL(BASE_URL + CATALOG_MENU_SPARE_PARTS_FOR_AGRICULTURAL_MACHINERY_URLs_END_POINTS[indx]);
			await expect(page.getByRole('link', { name: namePage, exact: true }).first()).toHaveText(namePage);
			await expect(page.locator('p.inline-block')).toHaveText(namePage);
		})
	...
	CATALOG_MENU_SPARE_PARTS_FOR_TRUCKS_CATEGORIES.forEach((namePage, indx) => {
		test(`TC 01.01.19.4 Verify that the user can navigate to ${namePage} page by clicking on the appropriate buttons in the "Запчастини до вантажних автомобілів" menu`, async ({ page }) => {
			const homePage = new HomePage(page);
			await homePage.clickCatalogbutton();
			await homePage.sparePartsForTrucksHover();
			await homePage.clickCatalogMenuButton2(namePage);

			await expect(page).toHaveURL(BASE_URL + CATALOG_MENU_SPARE_PARTS_FOR_TRUCKS_URLs_END_POINTS[indx]);
			await expect(page.getByRole('link', { name: namePage, exact: true }).first()).toHaveText(namePage);
			await expect(page.locator('p.inline-block')).toHaveText(namePage);
		})
